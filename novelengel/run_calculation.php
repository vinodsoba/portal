...
if($_POST['submit']){
	if(isset($_POST['item_id'])){
		$qry = "SELECT item_id, name, brand, price from ne_catalog where item_id like '$productId'";
		...
		while($row=$result->fetch(PDO::FETCH_ASSOC)){
			...
			$formatted = sprintf("%01.2f",	$priceperunit);
			 $name = "{$brand} {$prodname}";
			echo "<form  id='myForm' name='myForm' method='POST'><fieldset><tr><td><label>Product Code:</label></td><td><input type='text' id='sku' name='productcode' value='" .$productId ."'></td>";
		...
		$qry2 = "SELECT description, price, brand from ne_skincare_makeup where item_id like '$productId'";
